...
'					<button type="button" class="button" onclick="checkThenSendMail({idx}, false);"><img src="index.php?entryPoint=getImage&themeName=' + SUGAR.themes.theme_name + '&imageName=icon_email_send.gif" align="absmiddle" border="0"> {app_strings.LBL_EMAIL_SEND}</button>' +
...
/*Added by Suraj to check partner's contact in db*/
function checkThenSendMail(idx, flg) {
    // var myvar = getURLParameter('module');
    var myvar = $("input[name=parent_type]").val();
    if (myvar == 'kr_Dealers') {
        var pathname, tempArray, dealerId, toIds, ccIds, bccIds;
        dealerId = $("input[name=parent_id]").val();
        toIds = $("#addressTO0").val();
        toIds = toIds.split(",");
        ccIds = $("#addressCC0").val();
        ccIds = ccIds.split(",");
        for (var i = 0; i < ccIds.length; i++) {
            toIds.push(ccIds[i]);
        }
        bccIds = $("#addressBCC0").val();
        bccIds = bccIds.split(",");
        for (var i = 0; i < bccIds.length; i++) {
            toIds.push(bccIds[i]);
        }
        $.ajax({
            url: "index.php?module=kr_Assets&action=ajaxFun",
            data: {"mailIds": toIds, "dealerId": dealerId},
            type: "POST",
            datatype: "application/JSON",
            success: function(resp) {
                if ((jQuery.parseJSON(resp)).flag == 0) {
                    if (((jQuery.parseJSON(resp)).role) == 1) {
                        var retVal = confirm("One or more recipient(s) on this email is not a recognized contact. Do you still wish to continue? \n Click OK to Confirm or Cancel to abort ");
                        if (retVal == true) {
                            SUGAR.email2.composeLayout.sendEmail(idx, flg);
                        } else {
                            return false;
                        }
                    } else {
                        alert("One or more recipient(s) on this email is not a recognized contact. You can not send the mail.");
                    }
                }
                else {
                    SUGAR.email2.composeLayout.sendEmail(idx, flg);
...
/*to check the module before cheking the mail id in the database*/
