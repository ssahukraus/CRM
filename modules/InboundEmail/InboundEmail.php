...
//			_pp($UIDLs);_pp($cacheUIDLs);
	...
	function handleCaseAssignment($email) {
		...
		if($caseId = $this->getCaseIdFromCaseNumber($email->name, $c)) {
			$c->retrieve($caseId);
			if ( ($c->status=="Closed") || ($c->status=="Rejected") ) { //Alex
                        $c->status="ReOpened";
                        } else {
                         $c->status="FollowUp";
                        }
                         $c->save();
			// End of Alex email Follow Up
	...
	function handleCreateCase($email, $userId) {
		...
		if (!$this->handleCaseAssignment($email) && $this->isMailBoxTypeCreateCase()) {
			...
			$email->retrieve($email->id);
			$c = new aCase();
			/*Alex If Have open case from this email*/
			$new_case=1; //create new case?
			$skip_checking=0; // skip mailbox for attached into existing case?
			$email_from = $email->from_addr;
			
			
			$sql="SELECT c.id FROM cases c INNER JOIN  emails e on e.parent_id=c.id INNER JOIN emails_text et on et.email_id=e.id WHERE ( c.`status`='New' OR  c.`status`='FollowUp' ) AND et.from_addr LIKE '%".$email_from."%' ORDER BY c.date_entered DESC LIMIT 1";
			$results = $this->db->query($sql, true);
			$row_c = $this->db->fetchByAssoc($results);
 			
			$skip_emailboxes=$this->get_stored_options('create_case_skip_emails', ""); // skip mailboxes
			...
			if (is_array($row_c) && count($row_c)>0 &&$skip_checking==0){ 
				$new_case=0;
				$c->retrieve($row_c['id']);				
				$c->status="FollowUp";
				$c->priority = 'P1';
	...
	*/
		...
		foreach($parts as $k => $part) {
			...
			if(isset($part->parts) && !empty($part->parts) && !( isset($part->subtype) && strtolower($part->subtype) == 'rfc822')  ) {
				...
				if(strtolower($part->disposition) == 'attachment' || ((strtolower($part->disposition) == 'inline') && $part->type != 0)) {
					$GLOBALS['log']->fatal('If Loop #2 inside IF Inline attachment');
					...
					$GLOBALS['log']->fatal('If Loop #2 continie after renaiming');
					...
					if($forDisplay) {
						...
						$GLOBALS['log']->fatal('If Loop #2 for display=true');
					} else {
						// only save if doing a full import, else we want only the binaries
						$attach->save();
						$GLOBALS['log']->fatal('If Loop #2 only save if doing a full import, else we want only the binaries');
					}
			...
			{
				$GLOBALS['log']->fatal('If Loop #3 Retrieve contents of subtype rfc8822');
			    ...
			    $attach->safeAttachmentName();
				$GLOBALS['log']->fatal('If Loop #3 After Renaming');
			    if($forDisplay) {
			        $attach->id = $this->getTempFilename();

				$GLOBALS['log']->fatal('If Loop #3 FOr display=true');
			    } else {
			        // only save if doing a full import, else we want only the binaries
				$GLOBALS['log']->fatal('If Loop #3 only save if doing a full import, else we want only the binaries');
			        $attach->save();
			    }
				...
				$GLOBALS['log']->fatal('If Loop #4  No disposition here, but some IMAP servers');
				if($part->ifparameters) {
				 $GLOBALS['log']->fatal('If Loop #4  ifparameters=true');
                    foreach($part->parameters as $param) {
			$GLOBALS['log']->fatal('If Loop #4  param attribute');
			...
			$GLOBALS['log']->fatal('If Loop #4  filename is not empty ');
					// we assume that named parts are attachments too
                    $attach = $this->getNoteBeanForAttachment($emailId);

					$attach->filename = $attach->name = $fname;
					$attach->file_mime_type = $this->getMimeType($part->type, $part->subtype);
					$GLOBALS['log']->fatal('If Loop #4  deal with mime types ');
					$attach->safeAttachmentName();
					if($forDisplay) {
						 $GLOBALS['log']->fatal('If Loop #4 fordisplay=true ');
						$attach->id = $this->getTempFilename();
					} else {
						// only save if doing a full import, else we want only the binaries
						$attach->save();
						$GLOBALS['log']->fatal('If Loop #4 fordisplay=false ');
					}
				}
			$GLOBALS['log']->fatal('If Loop #4  end of if loop ');
			}
			$GLOBALS['log']->fatal('after if loops go inside savebinary');
			$this->saveAttachmentBinaries($attach, $msgNo, $thisBc, $part, $forDisplay);
		} // end foreach
		$GLOBALS['log']->fatal('End foreach func');
	}
	...
	function saveAttachmentBinaries($attach, $msgNo, $thisBc, $part, $forDisplay) {
		...
		$GLOBALS['log']->fatal('Inside Function saveAttachmentBinaries ');
		...
		if(!file_exists($uploadDir.$fileName)) {
			/*Alex*/
			$GLOBALS['log']->fatal('Attachment found and Saved: '.$uploadDir.$fileName);
			...
			if(file_put_contents($uploadDir.$fileName, $msgPart)) {
				...
				$GLOBALS['log']->fatal('attachement Saved ');
			} else {
                $GLOBALS['log']->debug('InboundEmail could not create attachment file: '.$attach->filename ." - temp file target: [ {$uploadDir}{$fileName} ]");
		$GLOBALS['log']->fatal('InboundEmail could not create attachment file:  '.$uploadDir.$fileName.' '.$attach->filename." - temp file target: [ {$uploadDir}{$fileName} ]");
                return;
			}
		}
		$GLOBALS['log']->fatal('If wasnt saved message file with attachment exists '.$uploadDir.$fileName);
		...
		/*Alex*/
		$GLOBALS['log']->fatal('Attachment Saving Completed if before was message that it found and saved: '.$uploadDir.$fileName);
	}
	...
	function importOneEmail($msgNo, $uid, $forDisplay=false, $clean_email=true) {
		...
		/*Alex*/
		$GLOBALS['log']->fatal("InboundEmail processing 1 email {$msgNo}-----------------------------------------------------------------------------------------");
		global $timedate;
		global $app_strings;
		global $app_list_strings;
		global $sugar_config;
		...
		if($forDisplay || $dupeCheckResult) {
			...
			if($structure->type == 1 && !empty($structure->parts)) {
				...
				/*Alex*/
				$GLOBALS['log']->fatal('InboundEmail found multipart email - saving attachments if found.  '.$email->id. " ");
				...
				if($uuemail) {
					/*Alex*/
					$GLOBALS['log']->fatal('InboundEmail found  UUEncoded attachments  - saving  it.  '.$email->id. " ");
					$email->description = $this->handleUUEncodedEmailBody($email->description, $email->id);
					$email->retrieve($email->id);
			   		$email->save();
		   		}
			} else {
				$GLOBALS['log']->fatal('InboundEmail not found attachments for #'.$email->id. " ");
				if($this->port != 110) {
			...
			/*Suraj for getting the inbound partners mail into their portal*/
			if(!empty($header->to)) {
				if($this->convertImapToSugarEmailAddress($header->to) == "ssahu@kraususa.com"){ //email ID need to change with the correspondent emailID as "partnersupport@kraususa.com"
					$email->intent	 = 'pick';
					$this->mailbox_type = 'pick';
					$from_mailId    =  $email->from_addr;
					$query = "SELECT eabr.bean_id, eabr.bean_module 
                              FROM email_addr_bean_rel eabr JOIN email_addresses ea
                              ON (ea.id = eabr.email_address_id)
                              WHERE eabr.deleted=0 AND ea.email_address = '$from_mailId'";
					$res = $this->db->query($query);
					while ($row = $GLOBALS['db']->fetchByAssoc($res)){
                       $contactId = $row['bean_id'];
                     }
					var_dump($contactId);
					$contact = BeanFactory::getBean('Contacts',$contactId);
					
					if(!empty($contact)){
						$partner = $contact->get_linked_beans('kr_dealers_contacts_1', 'kr_Dealers');
						$partner_id = "";
						foreach ($partner as $key => $val) {
							...
							if(!empty($partner))
							{ 
								$email->parent_id = $partner_id ;
								$email->parent_type = 'kr_Dealers';
