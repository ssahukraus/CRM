					...
					foreach($newMsgs as $k => $msgNo) {
						...
						if ($isGroupFolderExists) {
							if ($ieX->importOneEmail($msgNo, $uid)) {
								/*Alex*/
								$GLOBALS['log']->fatal('Email with Message # '.$msgNo. "Importing -------------------");
								// add to folder
								...
								if ($ieX->isMailBoxTypeCreateCase()) {
									/*Alex*/
									$GLOBALS['log']->fatal('Email # '.$msgNo. " is type Create case;");
									$userId = "";
									if ($distributionMethod == 'roundRobin') {
										/*Alex*/
										$GLOBALS['log']->fatal('Email # '.$msgNo. " is roundRobin");
										if (sizeof($users) == 1) {
											$userId = $users[0];
											$lastRobin = $users[0];
										} else {
										...
										/*Alex*/
										$GLOBALS['log']->fatal('Email # '.$msgNo. " is not roundRobin");
										if (sizeof($users) == 1) {
									...
									/*Alex*/
									$GLOBALS['log']->fatal('Email # '.$msgNo. " sent to create case------------------------------------------------------");
